...
def analyze_trend(symbol, historical_data, sentiment_score, signals, backtest_results, analysis_date):
    ...
    try:
        day3_return = float(backtest_results.get('day3_return', 0)) if backtest_results.get('day3_return') is not None else None
        day5_return = float(backtest_results.get('day5_return', 0)) if backtest_results.get('day5_return') is not None else None
    ...
    market_relative = {}
    if 'relative_day3_return' in backtest_results:
        market_relative['day3_relative_return'] = float(backtest_results['relative_day3_return'])
    if 'relative_day5_return' in backtest_results:
        market_relative['day5_relative_return'] = float(backtest_results['relative_day5_return'])
    if 'market_day3_return' in backtest_results:
        market_relative['market_day3_return'] = float(backtest_results['market_day3_return'])
    if 'market_day5_return' in backtest_results:
        market_relative['market_day5_return'] = float(backtest_results['market_day5_return'])
    ...
    trend_info = {
        ...
        'day3_direction': backtest_results.get('day3_direction', 'unknown'),
        'day5_return': day5_return,
        'day5_direction': backtest_results.get('day5_direction', 'unknown'),
...
def get_trend_insights(symbol):
    ...
    sentiment_3day_corr = analyze_correlation(
        [t.get('sentiment_score', 0) for t in trends if 'sentiment_score' in t and 'day3_return' in t], 
        [t.get('day3_return', 0) for t in trends if 'sentiment_score' in t and 'day3_return' in t]
    )
    
    sentiment_5day_corr = analyze_correlation(
        [t.get('sentiment_score', 0) for t in trends if 'sentiment_score' in t and 'day5_return' in t], 
        [t.get('day5_return', 0) for t in trends if 'sentiment_score' in t and 'day5_return' in t]
    )
    
    insights['sentiment_correlation'] = {
        '3day': sentiment_3day_corr,
        '5day': sentiment_5day_corr
    }
    ...
    rsi_trends = [t for t in trends if 'technical' in t and 'rsi' in t['technical'] and 'day3_return' in t]
    if rsi_trends:
        oversold_wins = sum(1 for t in rsi_trends if t['technical']['rsi'] < 30 and t['day3_return'] > 0)
        oversold_total = sum(1 for t in rsi_trends if t['technical']['rsi'] < 30)
        
        overbought_wins = sum(1 for t in rsi_trends if t['technical']['rsi'] > 70 and t['day3_return'] < 0)
    ...
    ma_trends = [t for t in trends if 'technical' in t and 'price_above_sma50' in t['technical'] and 'day3_return' in t]
    if ma_trends:
        above_sma50_wins = sum(1 for t in ma_trends if t['technical']['price_above_sma50'] and t['day3_return'] > 0)
        above_sma50_total = sum(1 for t in ma_trends if t['technical']['price_above_sma50'])
        
        below_sma50_wins = sum(1 for t in ma_trends if not t['technical']['price_above_sma50'] and t['day3_return'] < 0)
...
def calculate_basket_win_rate(symbols):
    ...
    wins_3day = sum(1 for trend in all_trends if trend.get('day3_direction') == 'up')
    wins_5day = sum(1 for trend in all_trends if trend.get('day5_direction') == 'up')
    
    # Calculate valid trades (ones with 3-day and 5-day data)
    valid_trades_3day = sum(1 for trend in all_trends if trend.get('day3_direction') in ['up', 'down'])
    valid_trades_5day = sum(1 for trend in all_trends if trend.get('day5_direction') in ['up', 'down'])
    ...
    avg_return_3day = sum(trend.get('day3_return', 0) for trend in all_trends if trend.get('day3_return') is not None) / valid_trades_3day if valid_trades_3day > 0 else 0
    avg_return_5day = sum(trend.get('day5_return', 0) for trend in all_trends if trend.get('day5_return') is not None) / valid_trades_5day if valid_trades_5day > 0 else 0
